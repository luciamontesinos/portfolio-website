...
const AnimatedLlogo = ({ strokeColor = '#fff' }) => {
  ...
  const renderEyes = () => {
    ...
    const dx = mousePos.x - logoCenterX;
    const dy = mousePos.y - logoCenterY;
    ...
    const leftEyeX = leftEyeCenterX + Math.cos(angle) * maxOffset;
    const leftEyeY = leftEyeCenterY + Math.sin(angle) * maxOffset;
    
    const rightEyeX = rightEyeCenterX + Math.cos(angle) * maxOffset;
    const rightEyeY = rightEyeCenterY + Math.sin(angle) * maxOffset;

    return (
      <>
        <circle cx={leftEyeX} cy={leftEyeY} r="5" fill={strokeColor} className="eye" />
        <circle cx={rightEyeX} cy={rightEyeY} r="5" fill={strokeColor} className="eye" />
      </>
    );
  };
  ...
  return (
    <>
      ...
      <div ref={logoRef} style={getContainerStyles()}>
        <svg
          viewBox="0 0 120 120"
          style={{
            width: '100%',
